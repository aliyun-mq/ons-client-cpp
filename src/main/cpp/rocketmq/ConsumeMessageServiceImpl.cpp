...
void ConsumeMessageServiceImpl::ack(const MQMessageExt& message, std::function<void(const std::error_code&)> cb) {
  auto consumer = consumer_.lock();
  if (!consumer) {
    return;
  }
  ...
  const auto& receipt_handle = message.receiptHandle();
...
void ConsumeMessageServiceImpl::nack(const MQMessageExt& message, std::function<void(const std::error_code&)> cb) {
  auto consumer = consumer_.lock();
  if (!consumer) {
    return;
  }
...
void ConsumeMessageServiceImpl::forward(const MQMessageExt& message, std::function<void(bool)> cb) {
  auto consumer = consumer_.lock();
  if (!consumer) {
    return;
  }
...
bool ConsumeMessageServiceImpl::preHandle(const MQMessageExt& message) {
  return true;
}

bool ConsumeMessageServiceImpl::postHandle(const MQMessageExt& message, ConsumeMessageResult result) {
  return true;
}
