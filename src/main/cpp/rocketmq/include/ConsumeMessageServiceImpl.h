...
public:
  ...
  bool preHandle(const MQMessageExt& message) override;

  bool postHandle(const MQMessageExt& message, ConsumeMessageResult result) override;

  void submit(std::shared_ptr<ConsumeTask> task) override;

  void dispatch(std::shared_ptr<ProcessQueue> process_queue, std::vector<MQMessageExt> messages) override;

  void ack(const MQMessageExt& message, std::function<void(const std::error_code&)> cb) override;

  void nack(const MQMessageExt& message, std::function<void(const std::error_code&)> cb) override;

  void forward(const MQMessageExt& message, std::function<void(bool)> cb) override;
